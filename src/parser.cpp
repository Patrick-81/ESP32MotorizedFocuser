...
// Gestion des boutons de raquette
#define movefromHandpad HIGH
...
//     {
// // PRINTDBG("pinstatus in =", int(digitalRead(INWARD)));
// // PRINTDBG("pinstatus out =", int(digitalRead(OUTWARD)));
// // delay(1000);
//       int t1 = millis();
//       int t2;
...
//         t2 = millis() - t1;
//       }
//       if (t2 > 2000)  // Retour en position 0
//       {
//         setnewPos(0);
//         go();
//       }
...
void speedplus()
{
  speedNumber = (speedNumber < 5 ? speedNumber+1 : speedNumber);
  wait = waitvalue[speedNumber-1];
}
void speedmoins()
{
  speedNumber = (speedNumber > 1 ? speedNumber-1 : speedNumber);
  wait = waitvalue[speedNumber-1];
}
// retour à zero
void gotozero()
...
///////////////////////////////////////////////////////////////////////////////
    // Commandes envoyées par la raquette (fonctionnel)
    if (!strcasecmp(cmd, "MI")) // move inward
    {
        interrupt = false;
        direction = -1;
        targetPos = (targetPos != 0 ? targetPos+direction : 0);
        PRINTDBG("%ld", targetPos);
    }
    if (!strcasecmp(cmd, "MF")) // move forward
    {
        interrupt = false;
        direction = +1;
        targetPos += direction;
